...
document.getElementById('hitchance-form').addEventListener('submit', function (e) 
{
    ...
    let attempts = parseFloat(document.getElementById('1attempt').value);
    ...
    let avghitchance =  calculateAverageHitChance(skillmod,enemyac, circum, status, item, circump, statusp, itemp, attempts,hitdiff);
    document.getElementById('avghc-result').textContent = avghitchance;

});
function calculateAverageHitChance(skillmod, enemyac, circum, status, item, circump, statusp, itemp, attempts, hitdiff)
{
